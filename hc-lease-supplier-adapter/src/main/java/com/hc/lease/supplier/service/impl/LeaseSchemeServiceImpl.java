...
import com.hc.lease.supplier.service.api.LeaseSchemeService;
            ...
            if (leaseSchemeList.size() > 0) {
                for (int i = 0; i < leaseSchemeList.size(); i++) {
                    ...
                    if (leaseScheme != null) {
                       ...
                       List<LeasePackageBalancePayment> leasePackageBalancePaymentList= leaseScheme.getLeasePackageBalancePayments();
                        if(leasePackageBalancePaymentList !=null && leasePackageBalancePaymentList.size()>0){
                           String  leasePackageBalancePaymentJson=JsonUtil.stringify(leasePackageBalancePaymentList);
                            leaseScheme.setLeasePackageBalancePaymentJson(leasePackageBalancePaymentJson);
                        }
                    }
                }
            }
