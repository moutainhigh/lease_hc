 ...
 * @version 2013-06-21
 */
public class DefinedFileUtils {
    private static String[] imageType = {"jpg", "png", "gif", "bmp", "jpeg"};
    private static Logger log = LoggerFactory.getLogger(DefinedFileUtils.class);
    ...
    public static boolean copyFile(String srcFileName, String descFileName) {
        return DefinedFileUtils.copyFileCover(srcFileName, descFileName, false);
    }
    ...
    public static boolean copyFileCover(String srcFileName, String descFileName, boolean coverlay) {
        ...
        if (descFile.exists()) {
            ...
            if (coverlay) {
                ...
                if (!DefinedFileUtils.delFile(descFileName)) {
    ...
    public static boolean copyDirectory(String srcDirName, String descDirName) {
        return DefinedFileUtils.copyDirectoryCover(srcDirName, descDirName, false);
    }
    ...
    public static boolean copyDirectoryCover(String srcDirName, String descDirName, boolean coverlay) {
        ...
        if (descDir.exists()) {
            if (coverlay) {
                ...
                if (!DefinedFileUtils.delFile(descDirNames)) {
        ...
        for (int i = 0; i < files.length; i++) {
            // 如果是一个单个文件，则直接复制
            if (files[i].isFile()) {
                flag = DefinedFileUtils.copyFile(files[i].getAbsolutePath(), descDirName + files[i].getName());
                // 如果拷贝文件失败，则退出循环
                if (!flag) {
                    break;
                }
            ...
            if (files[i].isDirectory()) {
                flag = DefinedFileUtils.copyDirectory(files[i].getAbsolutePath(), descDirName + files[i].getName());
                // 如果拷贝目录失败，则退出循环
                if (!flag) {
                    break;
                }
    ...
    public static boolean delFile(String fileName) {
        ...
        if (!file.exists()) {
            ...
            if (file.isFile()) {
                return DefinedFileUtils.deleteFile(fileName);
            } else {
                return DefinedFileUtils.deleteDirectory(fileName);
            }
        }
    }
    ...
    public static boolean deleteDirectory(String dirName) {
        ...
        for (int i = 0; i < files.length; i++) {
            // 删除子文件
            if (files[i].isFile()) {
                flag = DefinedFileUtils.deleteFile(files[i].getAbsolutePath());
                // 如果删除文件失败，则退出循环
                if (!flag) {
                    break;
                }
            ...
            // 删除子目录
                ...
                flag = DefinedFileUtils.deleteDirectory(files[i].getAbsolutePath());
                // 如果删除子目录失败，则退出循环
                if (!flag) {
                    break;
                }
     ...
     * 删除真实文件夹图片原图
     *
     * @param deleteImg 格式 "/upload/img1.jpg,/upload/img2.jpg"
     */
    public static void delReallyFile(String cata_log, String save_fileurl, String n_fileurl, String deleteImg) {
        if (deleteImg != null && !deleteImg.equals("")) {
            if (deleteImg.indexOf(",") > -1) {// 多张图片
                String[] img_obj = deleteImg.split(",");
                for (int i = 0; i < img_obj.length; i++) {
                    String img_url = img_obj[i].replaceAll(save_fileurl, cata_log + save_fileurl);
                    DefinedFileUtils.delFile(img_url);
                }
            } else {
                String img_url = deleteImg.replaceAll(save_fileurl, cata_log + save_fileurl);
                DefinedFileUtils.delFile(img_url);
            }
        }
    }
    ...
    public static void delTemporaryFile(String deleteImg) {
        if (deleteImg != null && !deleteImg.equals("")) {
            DefinedFileUtils.delFile(deleteImg);
        }
    }
